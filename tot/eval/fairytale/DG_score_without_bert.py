...
def update_gold_distractors(data):
    for item in data:
        ...
        # 确保 gold_distractors 的长度与 generated_distractors 一致
        if len(gold_distractors) < len(generated_distractors):
            gold_distractors.extend([''] * (len(generated_distractors) - len(gold_distractors)))

        updated_gold_distractors = gold_distractors.copy()
        ...
        for i, gen_distractor in enumerate(generated_distractors):
            ...
            if best_match:
                updated_gold_distractors[i] = best_match
        
        # 更新后的gold_distractors
        item['gold_distractors'] = updated_gold_distractors
    return data
...
def main():
    ...
    log_filename = f"DG_{dataset_name}_{num_data}_{now}.log"
