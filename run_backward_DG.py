...
from tot.methods.backward_DG_method import solve, naive_solve
...
def run(args):
    ...
    for i in range(args.task_start_index, args.task_end_index):
        ...
        if args.naive_run:
            ...
            info = naive_solve(args, task, i) 
        else:
            #ys, info = solve(args, task, i)
            info = solve(args, task, i)
...
def parse_args():
    ...
    args.add_argument('--task', type=str, required=True, choices=['QG', 'QDG','forward_QG','backward_QG','backward_DG'], default='QG')
    args.add_argument('--task_start_index', type=int, default=0)
    args.add_argument('--task_end_index', type=int, default=465)
